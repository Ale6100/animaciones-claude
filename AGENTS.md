...
- **Actualizar este archivo (`AGENTS.md`)**: si notás que una regla de trabajo cambió de forma duradera (no una excepción puntual de una sola tarea), actualizá este archivo y avisale al programador qué cambiaste. Integrá la regla nueva en la sección temática que corresponda, no suelta al final del archivo.
...
- **Procesos largos sin consultas periódicas**: cuando una tarea larga corre en segundo plano (un build, un render, un entrenamiento, una migración), lanzala con su salida redirigida a un log, avisale al programador cuánto estimás que tarda y terminá el turno. Volvé solo cuando llegue el aviso de que terminó: cada consulta intermedia obliga a procesar de nuevo todo el contexto de la conversación y gasta sin aportar nada. Si el programador quiere ver el avance, pasale el comando para que lo consulte él mismo sin involucrarte (por ejemplo, `! tail -1 <archivo de log>`).
...
- **Promover lo reutilizable**: si algo que vale la pena reutilizar (una función, un componente, un recurso, una utilidad) nace dentro de un archivo descartable, puntual o ignorado por git, generalizalo (parámetros en vez de valores fijos, sin dependencias de la tarea puntual) y llevalo al código compartido y versionado del proyecto, documentándolo donde el proyecto registre sus piezas reutilizables. Lo específico de una sola tarea se queda donde está. Así el conocimiento se acumula y no se reinventa en cada tarea.
...
- **Ciclo de vida efímero garantizado**: Si un archivo se crea exclusivamente como apoyo transitorio (por ejemplo, para que la IA inspeccione la pantalla, valide una salida o corra un test puntual), su ciclo de vida concluye en la misma iteración: se procesa/analiza y se purga del disco antes de emitir la respuesta final, salvo los intermedios de producción descritos más abajo.
...
- **Intermedios de producción costosos de regenerar**: los artefactos intermedios de un proceso largo (frames renderizados, builds, cachés de procesamiento) que permiten aplicar cambios sin rehacer todo el proceso no son descartables mientras el programador pueda pedir ajustes. Conservalos hasta que apruebe el resultado final, siempre dentro de una carpeta ignorada por git (listada en `.gitignore`) para que nunca terminen en un commit por error. Avisale cuánto ocupan y ofrecé borrarlos en cuanto lo apruebe.
